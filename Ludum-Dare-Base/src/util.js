base.registerModule('util', function() {
  ...
  var ExtendableTypedArray = extend(Object, 'ExtendableUint8Array', {
    constructor: function ExtendableTypedArray(type, partSize) {
      this.type = type;
      this.partSize = partSize || 1024;
      this.current = new this.type(this.partSize);
      this.index = 0;
      this.filled = [];
    },
  ...
  function createStateMachine(cfg) {
    var obj = {
      error: function(name, from, to, args, error, msg, e) {
        throw e || new Error(msg);
      }
    };
    var names = Object.getOwnPropertyNames(cfg);
    for(var i=0; i<names.length; i++) {
      var name = names[i];
      obj[name] = cfg[name];
    }
    return StateMachine.create(obj);
  }
  ...
  var SemiFilledTypedArray = extend(Object, 'SemiFilledTypedArray', {
    constructor: function SemiFilledTypedArray(type, capacity) {
      this.type = type;
  ...
  return {
    ...
    peek: peek,
    ExtendableTypedArray: ExtendableTypedArray,
    createStateMachine: createStateMachine,
    dist: dist,
    angle: angle,
    SemiFilledTypedArray: SemiFilledTypedArray
  };
});
